...
## About the Project
In space robotics, tools like the Satellite Dynamics Toolbox (SDTlib) [1] use Linear Fractional Transformations (LFTs) [2] to model spacecraft control systems with uncertain parameters. However, LFTs require strictly independent parameters to work. When a mechanism, such as a hexapod, has closed kinematic loops, its physical parts are constrained by one another, creating complex, non-rational mathematical constraints that cannot be directly modelled as LFTs. To address this, engineers must instead approximate the LFTs, which first requires the computation of the exact posture of the mechanism. To obtain this posture, and as a result approximate LFTs, complex constraint equations must be manually derived, which is typically a time-consuming and error-prone process.

This research provides a solution to this by proposing a parametric numerical solver that completely automates this process. Using a standardised JSON text file and graph theory [3, 4], the solver autonomously maps out a mechanism's loops and formulates the exact constraint equations. It then evaluates this constrained posture using the robust Levenberg-Marquardt algorithm [5]. Validated against the ESA Athena Hexapod platform [6] with a precision of 10⁻¹⁶ m, this tool automatically generates the exact numerical data required to build LFT approximations, working toward bridging the gap between complex physical robotics and accurate spacecraft modelling.
